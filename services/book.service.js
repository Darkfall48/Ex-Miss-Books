...
function query(filterBy = getDefaultFilter()) {
  return storageService.query(STORAGE_BOOK_KEY).then((books) => {
    if (filterBy.txt) {
...
function get(bookId) {
  return storageService.get(STORAGE_BOOK_KEY, bookId)
}

function remove(bookId) {
  return storageService.remove(STORAGE_BOOK_KEY, bookId)
}

function save(book) {
  if (book.id) {
    return storageService.put(STORAGE_BOOK_KEY, book)
  } else {
    return storageService.post(STORAGE_BOOK_KEY, book)
  }
}
...
function _createBooks() {
  let books = utilService.loadFromStorage(STORAGE_BOOK_KEY)
  if (!books || !books.length) {
    ...
    utilService.saveToStorage(STORAGE_BOOK_KEY, books)
  }
}
